...
class PacmanGame {
    constructor() {
        ...
        this.ghosts = [
            { x: 20, y: 10, direction: 'up', color: '#ff0000' },
            { x: 18, y: 12, direction: 'down', color: '#ffb8ff' },
            { x: 22, y: 12, direction: 'left', color: '#00ffff' },
            { x: 20, y: 14, direction: 'right', color: '#ffb852' }
        ];
    ...
    moveGhosts() {
        ...
        this.ghosts.forEach(ghost => {
            ...
            this.moveGhost(ghost, targetDirection);
        });
    }
    
    moveGhost(ghost, targetDirection) {
    ...
    gameOver() {
        ...
        // אם במסך מלא, צא ממנו קודם
        if (document.fullscreenElement) {
            document.exitFullscreen().then(() => {
                // הצג את הודעת המשחק נגמר רק אחרי יציאה ממסך מלא
                setTimeout(() => {
...
document.addEventListener('fullscreenchange', () => {
    ...
    if (!document.fullscreenElement) {
        ...
        canvas.style.padding = '';
        document.body.style.overflow = '';
        if (fullscreenBtn) fullscreenBtn.textContent = 'מסך מלא';
    } else {
        if (fullscreenBtn) fullscreenBtn.textContent = 'יציאה ממסך מלא';
    }
});
